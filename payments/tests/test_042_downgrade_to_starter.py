...
class DowngradeTestBase(IntegrationTestBase):
    ...
    def _assert_tier_unchanged(self) -> None:
        ...
        self._assert_subscription_equal(
            subscription,
            subscription=self.start_subscription,
    ...
    @mock.patch(
        "payments.stripewrapper.clear_all_payment_sources", return_value=(True, None)
    )
    def test_downgrade_ok(self, pmock: mock.Mock, smock: mock.Mock) -> None:
        ...
        pmock.assert_has_calls([mock.call(self.user.alumni.membership.customer)])
    ...
    @mock.patch("django.utils.timezone.now", mock.Mock(return_value=MOCKED_TIME))
    @mock.patch(
        "payments.stripewrapper.cancel_subscription",
        return_value=(None, "Testing Failure"),
    )
    @mock.patch(
        "payments.stripewrapper.clear_all_payment_sources", return_value=(True, None)
    )
    ...
    @mock.patch("payments.stripewrapper.cancel_subscription", return_value=(True, None))
    @mock.patch(
        "payments.stripewrapper.clear_all_payment_sources",
        return_value=(None, "Debug Fail"),
    )
...
class ContributorDowngradeTest(
    DowngradeTestBase, IntegrationTest, StaticLiveServerTestCase
):
