...
class UpgradeNoopTestBase(StripeFrontendTestMixin):
    ...
    def test_noop_ok(self) -> None:
        ...
        self.submit_form(
            "update_membership",
            "input_id_submit",
            select_dropdowns={"id_tier": TierField.get_description(self.start_tier)},
        )
...
class ContributorNoopTest(
    UpgradeNoopTestBase, IntegrationTest, StaticLiveServerTestCase
):
    fixtures = ["registry/tests/fixtures/integration.json"]
    user = "eilie"
