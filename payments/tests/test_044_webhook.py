...
def _stripe_mock(payload, *args):
    return (
        AttribMock(
            {
                "type": "payment_intent.mock",
                "data": AttribMock(
                    {"object": AttribMock(json.loads(payload)["object"])}
                ),
            }
        ),
        None,
...
class WebhookTest(TestCase):
    def test_rejects_unsigned(self):
        response = self.client.post(
            reverse("webhook"),
            data=SAMPLE_WEBHOOK_EVENT,
            content_type="application/json",
            HTTP_STRIPE_SIGNATURE="utter garbage",
        )
    ...
    def test_creates_local_object(self):
        ...
        self.assertTrue(
            PaymentIntent.objects.filter(
                stripe_id=SAMPLE_WEBHOOK_EVENT["object"]["id"]
            ).exists()
        )
    ...
    def test_loads_event_data(self):
        ...
        local_pi = PaymentIntent.objects.filter(
            stripe_id=SAMPLE_WEBHOOK_EVENT["object"]["id"]
        ).first()
