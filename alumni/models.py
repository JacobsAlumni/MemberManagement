...
class Alumni(
    AlumniSubscriptionMixin, AlumniEmailMixin, AlumniRegistryMixin, models.Model
):
    ...
    givenName: str = models.CharField(max_length=255, help_text="Your given name")
    middleName: Optional[str] = models.CharField(
        max_length=255, blank=True, null=True, help_text="Your middle name(s)"
    )
    familyName: str = models.CharField(max_length=255, help_text="Your family name")
    ...
    email: str = models.EmailField(help_text="Your private email address", unique=True)
    existingEmail: Optional[str] = models.EmailField(
        blank=True,
        null=True,
    ...
    birthday: date = models.DateField(help_text="Your birthday in YYYY-MM-DD format")
...
class Address(AlumniComponentMixin, models.Model):
    ...
    address_line_1: str = models.CharField(
        max_length=255, blank=True, null=True, help_text="E.g. Campus Ring 1"
    )
    address_line_2: Optional[str] = models.CharField(
        max_length=255, blank=True, null=True, help_text="E.g. Apt 007 (optional)"
    )
    ...
    state: Optional[str] = models.CharField(
        max_length=255, blank=True, null=True, help_text="E.g. Bremen (optional)"
    )
    country: Country = fields.CountryField(blank=True, null=True)

    @property
    def coords(
        self, default: Optional[Union[List[float], List[None]]] = None
    ) -> Union[List[float], List[None]]:
        """The coordinates of this user"""
        from atlas.models import GeoLocation

        lat, lng = GeoLocation.getLoc(
            self.country, self.zip, self.member.atlas.reducedAccuracy
        )
...
class SocialMedia(AlumniComponentMixin, models.Model):
    ...
    facebook: Optional[str] = models.URLField(
        null=True, blank=True, help_text="Your Facebook Profile (optional)"
    )
    linkedin: Optional[str] = models.URLField(
        null=True, blank=True, help_text="Your LinkedIn Profile (optional)"
    )
    twitter: Optional[str] = models.URLField(
        null=True, blank=True, help_text="Your Twitter Account (optional)"
    )
    instagram: Optional[str] = models.URLField(
        null=True, blank=True, help_text="Your Instagram (optional)"
    )
    homepage: Optional[str] = models.URLField(
        null=True, blank=True, help_text="Your Homepage or Blog"
    )
...
class JacobsData(AlumniComponentMixin, models.Model):
    ...
    def is_filled(self) -> bool:
        ...
        return (self.graduation != fields.ClassField.OTHER) and (
            self.major != fields.MajorField.OTHER
        )
    ...
    comments: Optional[str] = models.TextField(
...
class Approval(models.Model):
    ...
    autocreated: bool = models.BooleanField(
        default=False, blank=False, help_text="Has the user been automatically created?"
    )

    approval: bool = models.BooleanField(
        default=False, blank=True, help_text="Has the user been approved by an admin?"
    )

    gsuite: Optional[str] = models.EmailField(
        blank=True, null=True, help_text="The G-Suite E-Mail of the user", unique=True
    )
...
class JobInformation(AlumniComponentMixin, models.Model):
    ...
    employer: Optional[str] = models.CharField(
        max_length=255, null=True, blank=True, help_text="Your employer (optional)"
    )
    position: Optional[str] = models.CharField(
        max_length=255, null=True, blank=True, help_text="Your position (optional)"
    )
...
class Skills(AlumniComponentMixin, models.Model):
    ...
    programmingLanguages: Optional[str] = models.TextField(null=True, blank=True)
    areasOfInterest: Optional[str] = models.TextField(
        null=True,
        blank=True,
        help_text="E.g. Start-Ups, Surfing, Big Data, Human Rights, etc",
    )
    alumniMentor: bool = models.BooleanField(
        default=False,
        blank=True,
