...
class AddressTest(IntegrationTest, StaticLiveServerTestCase):
    ...
    def test_signup_address_minimal(self) -> None:
        self.submit_form(
            ...
            select_dropdowns={"id_country": ("DE",)},
        )

        self.assert_url_equal(
            "setup_social", "Check that the user gets redirected to the social page"
        )
    ...
    def test_signup_address_full(self) -> None:
        self.submit_form(
            ...
            select_dropdowns={"id_country": ("US",)},
        )
    ...
    def test_signup_address_fail(self) -> None:
        button = self.fill_out_form("setup_address", "input_id_submit")
        ...
        self.assert_url_equal(
            "setup_address", "Check that the user stays on the address page"
        )

        for id_ in ["id_address_line_1", "id_zip", "id_city", "id_country"]:
            self.assertIn(
                "uk-form-danger",
                self.selenium.find_element(By.ID, id_)
                .get_attribute("class")
                .split(" "),
                "{} field marked up as incorrect".format(id_),
            )
