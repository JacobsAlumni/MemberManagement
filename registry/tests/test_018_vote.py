...
class VoteLinkTest(IntegrationTest, StaticLiveServerTestCase):
    ...
    def test_create_tokens(self) -> None:
        ...
        self.assertIsNotNone(token, "check that get_token returns a token object")
    ...
    def test_export_tokens(self) -> None:
        ...
        self.assertNotEqual(
            got_tokens, expect_tokens, "Non-superuser can not download tokens"
        )
    ...
    def test_show_tokens_approved(self) -> None:
        ...
        Active_Without_Token = str(VoteLink.objects.get(pk=2).get_token(Mounfem).token)

        # check the first link
        vote_link = self.find_element("#id_vote_link_1")
        self.assertEqual(
            vote_link.get_attribute("href"),
            "https://example.com/active-token/?token={}".format(Active_Token),
            "Check that the first link is set correctly",
        )
        self.assert_element_exists(
            "#id_vote_link_1_personalized",
            "Check that the first link is marked as personalized",
        )
        vote_token = self.find_element("#id_vote_token_1")
        self.assertEqual(
            vote_token.get_attribute("value"),
            Active_Token,
            "Check that the first token is set correctly",
        )
        ...
        self.assertEqual(
            vote_link.get_attribute("href"),
            "https://example.com/active-no-token/",
            "Check that the second link is set correctly",
        )
        self.assert_element_not_exists(
            "#id_vote_link_2_personalized",
            "Check that the second link is not marked as personalized",
        )
        vote_token = self.find_element("#id_vote_token_2")
        self.assertEqual(
            vote_token.get_attribute("value"),
            Active_Without_Token,
            "Check that the second token is set correctly",
        )
    ...
    def test_show_tokens_unapproved(self) -> None:
        ...
        self.assertEqual(
            VoteToken.objects.count(), 0, "check that no tokens were created"
        )
