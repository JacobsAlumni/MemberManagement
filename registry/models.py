...
class VoteLink(models.Model):
    active: bool = models.BooleanField(help_text="Enable showing this vote link", default=False)
    ...
    url: str = models.URLField(help_text="URL where the vote takes place. Use ${token} as a placeholder for the token. ")
...
class VoteToken(models.Model):
    ...
    vote: VoteLink = models.ForeignKey(VoteLink, on_delete=models.CASCADE, help_text="Vote this Token is associated to")
    alumni: Alumni = models.ForeignKey(Alumni, on_delete=models.CASCADE, help_text="Alumni this token is for")
